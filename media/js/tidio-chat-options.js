var tidioLiveOptions = {
	...
	initEvents: function(){
		...
		$("#live-settings-link").on('click', function(){
	...
	apiUpdateSettings: function(_func){
		...
		var plugin_data = tidioLiveOptions.apiDataSerialize('#dialog-settings-form');
		...
		$.ajax({
			url: this.ajax_url + 'admin-ajax.php?action=tidio_live_settings_update',
			type: 'POST',
			data: {
	...
	liveLoading: function(){
		...
		$("#live-content").html(
			'<iframe src="' + this.plugin_url + 'en/panel/plugins/live/external?externalAccessKey=' + this.private_key + '" id="live-iframe" style="height: ' + iframe_height + 'px;"></iframe>'
		);		
		
		$("#live-iframe").load(function(){
			
			$("#live-loading").fadeOut('fast', function(){
				
				$("#live-content").fadeIn('fast');
				
				tidioLiveOptions.liveIsReady();
